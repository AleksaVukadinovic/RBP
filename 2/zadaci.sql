...
FROM DA.PREDMET P1
         JOIN
     DA.PREDMET P2 ON P1.ESPB = P2.ESPB
WHERE P1.ID <> P2.ID;
...
SELECT P.NAZIV, PP.IDPROGRAMA, P.ID, PP.VRSTA, UP.IDUSLOVNOGPREDMETA
FROM DA.PREDMETPROGRAMA PP
         LEFT JOIN DA.USLOVNIPREDMET UP ON PP.IDPREDMETA = UP.IDPREDMETA AND PP.IDPROGRAMA = UP.IDPROGRAMA
         JOIN DA.PREDMET P ON UP.IDUSLOVNOGPREDMETA = P.ID
